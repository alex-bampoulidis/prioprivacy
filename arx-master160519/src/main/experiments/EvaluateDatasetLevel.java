...
public class EvaluateDatasetLevel {
	...
	private static String output = outputPath + "results_dataset_level.csv";
	...
	public static void main(String args[]) throws Exception {
		...
		bw.write(
				"k,gen. intensity,granularity,n.u. entropy,discernibility,avg. class size,record-level sp. err, attribute-level sq. err.,experiment,method\n");
		...
		for (String method : methods) {
			for (int experiment = 1; experiment <= 4; experiment++) {
				for (int k = 2; k <= 10; k++) {
					...
					bw.write(statQ.getDiscernibility().getValue() + ",");
					bw.write(statQ.getAverageClassSize().getValue() + ",");
